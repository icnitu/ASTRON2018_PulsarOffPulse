...
def fitGauss_ACon(ACx_on):
	...
	x0_guess = int(Nx/2)
	sigma_guess = np.sqrt(np.sum((ACxfit-yoffset_guess)*((xfit-x0_guess)**2))/float(np.sum(ACxfit-yoffset_guess)))
...
def fit2Gauss_ACon(ACx_on):
	...
	yoffset_guess = min(ACxfit)
	x0_guess = int(Nx/2)
	sigma1_guess = np.sqrt(np.sum((ACxfit-yoffset_guess)*((xfit-x0_guess)**2))/float(np.sum(ACxfit-yoffset_guess)))
